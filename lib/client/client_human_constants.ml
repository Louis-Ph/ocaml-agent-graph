...
module Wizard = struct
  let topics = [ "build"; "test"; "install"; "cron"; "swarm"; "ssh" ]
end
...
module Text = struct
  ...
  let intro_lines =
    [
      ...
      "ocaml-agent-graph adds typed orchestration, graph control, and smarter swarms on top.";
      "The human assistant should guide build, test, install, cron, SSH, and swarm operations.";
    ]
  ...
  let command_help_lines current_route_model =
    [
      ...
      "  /docs TOPIC show the most relevant local docs for build, test, install, cron, swarm, or ssh";
  ...
  let tool_lines =
    [
      ...
      "  swarm   -> ask /wizard swarm ... and inspect the adaptive webcrawler or worker mode";
      "  ssh     -> inspect /ssh-human and /ssh-machine before remote execution";
      "  docs    -> use /docs build, /docs swarm, /docs ssh, or /docs cron";
    ]
