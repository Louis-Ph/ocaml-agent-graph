...
let rec loop ~config ~registry context payload =
  ...
  | Next agent ->
      let* item = Runtime_engine.run_agent ~config ~registry agent context payload in
      let next_context = Core_context.record_outcome context item in
      loop ~config ~registry next_context item.payload
  | Parallel agents ->
      let* items =
      ...
      loop ~config ~registry next_context merged_payload
