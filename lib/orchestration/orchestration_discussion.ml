...
module Live_output = struct
  ...
  let started_message (config : Runtime_config.Discussion.t) =
    Fmt.str "Discussion started: rounds=%d final_agent=%s participants=%s"
      config.rounds
  ...
  let round_started_message ~round_index ~max_rounds ~participants =
    Fmt.str "Discussion round %d/%d started with %s" round_index max_rounds
      (participant_names participants)

  let round_completed_message ~round_index ~max_rounds ~turn_count =
    Fmt.str "Discussion round %d/%d completed with %d turn(s)" round_index
      max_rounds turn_count

  let turn_completed_message ~max_rounds (turn : Core_payload.discussion_turn) =
    Fmt.str "Discussion turn %d/%d speaker=%s\n%s" turn.round_index max_rounds
      turn.speaker
      (indent_block turn.content)

  let turn_failed_message ~round_index ~max_rounds ~speaker ~error =
    Fmt.str "Discussion turn %d/%d speaker=%s failed: %s" round_index max_rounds
      speaker error

  let turn_skipped_message ~round_index ~max_rounds ~speaker ~route_model
      ~ready_backends =
    Fmt.str
      "Discussion turn %d/%d speaker=%s skipped: empty completion \
       (route_model=%s ready_backends=%d)"
      round_index max_rounds speaker route_model ready_backends

  let budget_exhausted_message ~collected_turns ~message =
    Fmt.str "Discussion halted: budget exhausted after %d turn(s) collected. %s"
      collected_turns message

  let unready_route_message ~participant_name ~route_model =
    Fmt.str
      "Discussion participant %s: route_model=%s has no ready backends — all \
       API keys missing, turns will return empty"
      participant_name route_model
end

module Prompt_templates = struct
  let default_max_contribution_words = 120
  let hard_max_contribution_words = 380
  let local_max_contribution_words = hard_max_contribution_words
  ...
  let max_contribution_words_for_participant
    ...
    if is_local_route_model participant.profile.route_model then
      local_max_contribution_words
  ...
  let versioned_block label = function
    ...
    | Some (entry : Runtime_config.Discussion.Versioned_text.t) ->
        Some (Fmt.str "%s (version %s)\n%s" label entry.version entry.text)
  ...
  let render_system_prompt
    ...
    |> List.filter_map (fun value ->
           value |> Option.map String.trim |> function
           | Some "" | None -> None
  ...
  let build_messages ~(participant : Runtime_config.Discussion.Participant.t)
      ~context:(_context : Core_context.t)
      ~(discussion : Core_payload.discussion) ~round_index:_ =
    ...
    let user_message : Bulkhead_lm.Openai_types.message =
      {
        role = "user";
        content =
          (if transcript_block = "" then
             Fmt.str
               "Topic: %s\n\n\
                Agenda:\n\
                %s\n\n\
                You are %s. Give your initial contribution. Be direct and \
                substantive. No meta-commentary. Stay under %d words. Only \
                emit %s as the very first token if the topic is trivially \
                settled; otherwise never emit that marker."
               discussion.topic
               (render_agenda discussion.agenda)
               participant.name max_contribution_words convergence_marker
           else
             Fmt.str
               "Topic: %s\n\n\
                Agenda:\n\
                ...
                %s\n\n\
                You are %s. Add your next contribution. Build on what was said \
                when useful. Be direct and substantive. No meta-commentary. \
                Stay under %d words. Only emit %s as the very first token if \
                you independently judge the discussion materially settled; \
                otherwise never emit that marker."
               discussion.topic
               (render_agenda discussion.agenda)
...
let trim_to_max_words ~max_words content =
  let words =
    let buffer = Buffer.create (String.length content) in
    let flush acc =
      let word = Buffer.contents buffer |> String.trim in
      Buffer.clear buffer;
      if word = "" then acc else word :: acc
    in
    let acc =
      String.fold_left
        (fun acc ch ->
  ...
  in
  take max_words [] words |> String.concat " "
...
let first_user_message (context : Core_context.t) =
  ...
  in
  context.Core_context.history |> List.rev |> loop

let discussion_of_payload (config : Runtime_config.Discussion.t)
    (context : Core_context.t) = function
  ...
  | payload ->
      Error
        (Fmt.str "Discussion workflow cannot process %s"
           (Core_payload.summary payload))

let participant_metrics (profile : Runtime_config.Llm.Agent_profile.t)
    (completion : Llm_bulkhead_client.completion) =
  ( { Core_payload.confidence = profile.confidence; cost = 0.0; latency_ms = 0 },
    [
      Fmt.str
        "Discussion participant route_model=%s resolved_model=%s \
         prompt_tokens=%d completion_tokens=%d total_tokens=%d."
        completion.route_model completion.model completion.usage.prompt_tokens
...
let is_budget_exhausted message =
  ...
  String.length message >= plen && String.sub message 0 plen = prefix
...
let meta_token_score value =
  ...
  |> List.fold_left
       (fun score token ->
         if contains_substring ~substring:token lowered then score + 1
         else score)
       0

let line_repeats_participant_guidance
    (participant : Runtime_config.Discussion.Participant.t) line =
  let lowered_line = String.lowercase_ascii line in
  let guidance =
    Prompt_templates.render_system_prompt participant |> String.lowercase_ascii
  in
  let tokenize text =
...
let normalize_contribution_content
    ~(participant : Runtime_config.Discussion.Participant.t) content =
  let trimmed = String.trim content in
  if trimmed = "" then ""
  else
    let marker_prefix, body =
      if has_leading_convergence_marker trimmed then
        let marker = Prompt_templates.convergence_marker in
        let marker_length = String.length marker in
        let body =
          String.sub trimmed marker_length
            (String.length trimmed - marker_length)
          |> String.trim
        in
        (marker, body)
      else ("", trimmed)
    ...
    let normalized_lines =
      ...
      |> List.filter (fun line ->
             ...
             not
               ...
               || String.starts_with ~prefix:"**stage " lowered
               || String.ends_with ~suffix:":" lowered
                  && meta_token_score lowered >= 1
    ...
    let normalized_lines =
      ...
      | lines ->
          ...
          take 6 [] lines
    in
    let normalized = String.concat "\n" normalized_lines |> String.trim in
    let normalized =
      trim_to_max_words ~max_words:Prompt_templates.hard_max_contribution_words
        normalized
      |> String.trim
    in
    if marker_prefix = "" then normalized else marker_prefix ^ "\n" ^ normalized

let invoke_participant ~(services : Runtime_services.t)
    ~(context : Core_context.t) ~(discussion : Core_payload.discussion)
    ~(participant : Runtime_config.Discussion.Participant.t) ~round_index =
  let profile = participant.profile in
  let* result =
    Llm_bulkhead_client.invoke_messages services.llm_client
      ~route_model:profile.route_model
      ~messages:
        (Prompt_templates.build_messages ~participant ~context ~discussion
           ~round_index)
      ~max_tokens:profile.max_tokens
  in
  match result with
  | Error message when is_budget_exhausted message ->
      Runtime_logger.log Runtime_logger.Error
        (Live_output.turn_failed_message ~round_index
           ~max_rounds:discussion.max_rounds ~speaker:participant.name
           ~error:message);
      let context =
        Core_context.record_event context ~label:Event_label.turn_failed
          ~detail:
            (Fmt.str "round=%d speaker=%s error=%s" round_index participant.name
               message)
      in
  ...
  | Ok completion ->
      ...
      if content = "" then
        ...
        let () =
          Runtime_logger.log Runtime_logger.Warning
            (Live_output.turn_skipped_message ~round_index
               ~max_rounds:discussion.max_rounds ~speaker:participant.name
               ~route_model:route_access.route_model
               ~ready_backends:route_access.ready_backend_count)
        in
        let context =
          Core_context.record_event context ~label:Event_label.turn_skipped
            ~detail:
              (Fmt.str "round=%d speaker=%s reason=empty_completion" round_index
                 participant.name)
        in
        Lwt.return (Turn_skipped, context)
      else
        ...
        in
        let context = Core_context.record_discussion_turn context turn in
        Runtime_logger.log Runtime_logger.Info
          (Live_output.turn_completed_message ~max_rounds:discussion.max_rounds
             turn);
        Lwt.return (Turn_produced turn, context)

let round_has_converged (discussion : Core_payload.discussion) ~round_index
    ~participant_count =
...
let extract_sub_topic content =
  ...
  let rec scan pos =
    if pos >= String.length content then None
      ...
      if
        String.length remaining >= marker_len
        && String.sub remaining 0 marker_len = open_marker
      then
        let after_marker =
          String.sub remaining marker_len (String.length remaining - marker_len)
        in
...
let rec run_round ~(services : Runtime_services.t) ~(config : Runtime_config.t)
    ~(participants : Runtime_config.Discussion.Participant.t list)
    ~(round_index : int) context (discussion : Core_payload.discussion) =
  let rec loop context (discussion : Core_payload.discussion)
      produced_turn_count = function
    | [] -> Lwt.return (`Ok, discussion, context, produced_turn_count)
    | participant :: rest -> (
        let* turn_result, context =
          invoke_participant ~services ~context ~discussion ~participant
            ~round_index
        in
        ...
        | Turn_budget_exhausted message ->
            Lwt.return
              ( `Budget_exhausted message,
                ...
                context,
                produced_turn_count )
        | Turn_skipped -> loop context discussion produced_turn_count rest
        | Turn_produced turn ->
            let discussion =
            ...
            let* discussion =
              if
                config.discussion.max_nesting_depth
                <= context.Core_context.nesting_depth
              then Lwt.return discussion
                ...
                | Some sub_topic ->
                    Runtime_logger.log Runtime_logger.Info
                      (Fmt.str "Spawning sub-discussion: %s (requested by %s)"
                         sub_topic turn.speaker);
                    let child_task_id =
                      Fmt.str "%s/sub-%d-%s" context.task_id round_index
                        turn.speaker
                    in
                    let child_context =
                    ...
                    let* sub_payload, _sub_context =
                      run_sub ~services ~config child_context sub_plan
                    in
                    let sub_disc =
                    ...
                    Lwt.return
                      {
                        discussion with
                        sub_discussions =
                          discussion.sub_discussions @ [ sub_entry ];
                      }
            in
...
and run_sub ~services ~config context payload =
  ...
  | Ok discussion ->
      let rec sub_loop context (discussion : Core_payload.discussion)
          round_index =
        if round_index > discussion.max_rounds then
          Lwt.return (Core_payload.Discussion discussion, context)
        else
          let* round_status, discussion, context, turn_count =
            run_round ~services ~config:sub_config
              ~participants:sub_config.discussion.participants ~round_index
              context discussion
          in
          let discussion = { discussion with completed_rounds = round_index } in
          let context =
            Core_context.record_discussion_round context ~round_index
              ~turn_count
          in
...
let warn_unready_participant_routes (llm_client : Llm_bulkhead_client.t)
    (participants : Runtime_config.Discussion.Participant.t list) =
  participants
         ...
         | Some access when access.ready_backend_count = 0 ->
             Runtime_logger.log Runtime_logger.Warning
               (Live_output.unready_route_message
                  ~participant_name:participant.name ~route_model)
         | _ -> ())

let run ~(services : Runtime_services.t) ~(config : Runtime_config.t) context
    payload =
  match discussion_of_payload config.discussion context payload with
  | Error message ->
      let context =
        Core_context.record_event context ~label:Event_label.failed
          ~detail:message
      in
  ...
  | Ok discussion ->
      ...
      let context =
        Core_context.record_event context ~label:Event_label.started
          ~detail:
            (Fmt.str "rounds=%d final_agent=%s participants=%s"
      ...
      warn_unready_participant_routes services.llm_client
        config.discussion.participants;
      let rec loop context (discussion : Core_payload.discussion) round_index =
        if round_index > discussion.max_rounds then
          ...
          let () =
            Runtime_logger.log Runtime_logger.Info
              (Live_output.round_started_message ~round_index
                 ~max_rounds:discussion.max_rounds
                 ~participants:config.discussion.participants)
          in
          let context =
            Core_context.record_event context ~label:Event_label.round_started
              ~detail:(Fmt.str "round=%d" round_index)
          in
          let* round_status, discussion, context, turn_count =
            run_round ~services ~config
              ~participants:config.discussion.participants ~round_index context
              discussion
          in
          let discussion = { discussion with completed_rounds = round_index } in
          let context =
          ...
          in
          Runtime_logger.log Runtime_logger.Info
            (Live_output.round_completed_message ~round_index
               ~max_rounds:discussion.max_rounds ~turn_count);
          match round_status with
          | `Budget_exhausted message ->
              Runtime_logger.log Runtime_logger.Warning
          ...
          | `Ok ->
              if turn_count = 0 then
                ...
                round_has_converged discussion ~round_index
                  ~participant_count:
                    (List.length config.discussion.participants)
              then (
                Runtime_logger.log Runtime_logger.Info
                  (Fmt.str
                     "Discussion converged at round %d/%d — majority emitted \
                      the explicit convergence marker."
                     round_index discussion.max_rounds);
                Lwt.return (Core_payload.Discussion discussion, context))
